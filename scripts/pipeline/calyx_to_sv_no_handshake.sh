...
promote_invoked_toplevel="${CALYX_PROMOTE_INVOKED_TOPLEVEL:-}"
...
if [[ -n "$promote_invoked_toplevel" ]]; then
  python3 "$promote_invoked_toplevel" \
    "$output_dir/model.futil" "$output_dir/model.promoted.futil" \
    >"$output_dir/logs/promote-calyx-invoked-toplevel.log" 2>&1
  mv "$output_dir/model.promoted.futil" "$output_dir/model.futil"
fi
