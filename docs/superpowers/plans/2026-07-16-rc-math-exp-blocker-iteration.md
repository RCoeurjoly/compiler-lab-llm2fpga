...
- If the complete non-black-boxed baseline fails constrained P&R, publish its failure packet before beginning any DDR3 work. An early DDR3 route is then a separately labelled non-baseline branch with its own gates, never a substitute for the baseline result.
- Treat all successful V=6 vertical-slice results as RC-only. Do not extrapolate full TinyStories fit/performance until a separately recorded controlled frozen-full-versus-exact-finalist comparison measures structural and downstream resource scaling under the same route/tool versions.
- As soon as any testable RC SV exists, run a bounded deterministic verification-throughput probe and publish its context count/range, simulator/host provenance, wall time, contexts/sec, cycles/context, available peak resources, and projected full `6^8` sweep cost. It is feasibility evidence, not a substitute for exhaustive promotion.
- If the full-sweep projection is impractical, publish a verification-scalability blocker with the probe, attempted strategy, and limiting resource. Keep every candidate provisional; improve/shard/parallelize verification or investigate a stronger proof instead of replacing exhaustive promotion with sampled testing.
