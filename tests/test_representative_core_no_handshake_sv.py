...
class RepresentativeCoreNoHandshakeSvTest(unittest.TestCase):
    ...
    def test_pre_calyx_uses_checked_in_mlir_pass_plugin(self) -> None:
        ...
        self.assertIn("calyxPreflightReport = pkgs.substituteAll", pipeline)
        self.assertIn("calyxPreflightMlirOptPath", pipeline)
        self.assertIn("calyxPreflightMlirOptVersion", pipeline)
        self.assertIn("calyxPreflightMlirOptSha256", pipeline)
        self.assertIn('builtins.hashFile "sha256"', pipeline)
        self.assertIn("${mlir}/bin/mlir-opt", pipeline)
        self.assertNotIn("CALYX_PREFLIGHT_MLIR_OPT", pipeline)
