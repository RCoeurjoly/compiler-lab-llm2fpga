...
**Complete transformer block**: The compiler-owned four-row block path from
block input through attention, MLP, and the final MLP residual addition. Its
...
**Dual-plane instrumentation**:
The optimization evidence combines static FPGA implementation reports (area, timing, and critical paths) with dynamic simulation or trace counters (cycles, operations, stalls, memory traffic, and bandwidth) for the same candidate and workload.
_Avoid_: static-only bottleneck, simulation-only area claim, mismatched workload receipts

**Measured model-fit boundary**:
...
**RTL-qualified candidate**:
A candidate that passes PyTorch-backed simulation, EQY (when applicable), Yosys, nextpnr-xilinx, and target resource-fit checks but has not yet been validated on physical hardware.
_Avoid_: hardware-qualified, board-proven performance

**Hardware-qualified candidate**:
An RTL-qualified candidate that also executes successfully on the target board and has measured physical timing, throughput, and DDR3 behavior recorded in its receipt.
_Avoid_: simulation-only qualification, unmeasured board claim

**Hardware artifact identity**:
