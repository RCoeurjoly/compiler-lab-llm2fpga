...
## Final capture-infrastructure fix wave

Nonzero registered builds no longer become compiler frontiers merely because a Nix wrapper printed an `error:` line. The classifier now returns a distinct `EnvironmentFailure` for daemon/socket, permission, sandbox, store, substituter/fetch, evaluation, build-wrapper, and otherwise unattributed nonzero failures. The capture runner aborts immediately on that result. `CompilerFailure` is reserved for a diagnostic attributable to compiler semantics, the bound upstream MLIR location, or a bound invoked tool; operation and type facts continue to be derived only from the attributed diagnostic. A trailing generic Nix builder summary does not erase an independently attributable compiler diagnostic.

The public v5 verifier now dispatches its exact recursive schema and live semantics by the `frontier_evidence` discriminator. The residual control-manifest schema and c22 live bindings are unchanged. The compiler-failure branch requires a nonzero live replay, a null build result and manifest, exact bound upstream artifact, independently derived non-environmental diagnostic/operation/types, exact stage/execution/log/tool fields, classifier interestingness fields and replay, and the branch-specific minimization/file set. The public projection follows the verified first-invalid stage rather than assuming `flat-scf`.

Tests construct a real two-run compiler-failure bundle on disk from the classifier's `CompilerFailure` serialization and generated interestingness script, then enter through `verify_public_v5_evidence`. The canonical fixture passes; recomputed/self-consistent mutations covering crossed branch files, manifest injection, missing log/input/tool binding, zero exit, the reviewer daemon-socket diagnostic, operation/type lies, combined lies, and extra/missing/symlink/subdirectory/nonregular entries fail closed. Representative daemon, permission, sandbox, store, substituter, evaluation, wrapper-only, and unattributed logs also exercise classifier rejection and the capture-abort boundary.

This wave changed capture infrastructure, verifier logic, tests, this report, and the ledger only. It did not rebuild or recapture SCF and did not modify any c22 receipt, bundle, reproducer, compiler, model, adapter, or route payload. Cached c22 public verification still reports 20 canonical files, 19 public reproducer files, and first invalid `flat-scf` with the original receipt hashes.

The four focused registration/semantic/classifier/determinism modules pass 134 tests in the Nix development environment. The standalone cached public verifier and `nix flake check --no-build` pass, as do Python compilation, `git diff --check`, and the repository pre-final check after commit.
